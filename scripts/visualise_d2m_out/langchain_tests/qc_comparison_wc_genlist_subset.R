...
wcharlist_lc_sg_qc <- read_tsv(here::here("../outputs/qc_output/wcharlist/wcharlist_langchain_sgenlist_subset_qc.tsv")) %>%
  dplyr::rename(id = 1) %>% # Rename first column as index
  mutate(mode = "wcharlist_lc_sg") # Specify run mode
...
wcharlist_lc_lg_qc <- read_tsv(here::here("../outputs/qc_output/wcharlist/wcharlist_langchain_lgenlist_subset_qc.tsv")) %>%
  dplyr::rename(id = 1) %>% # Rename first column as index
  mutate(mode = "wcharlist_lc_lg") # Specify run mode

# Merge dataframes
qc_df <- rbind(wcharlist_sg_qc, wcharlist_f_sg_qc, wcharlist_lc_sg_qc, wcharlist_lg_qc, wcharlist_f_lg_qc, wcharlist_lc_lg_qc) %>%
...
# Relevel mode names
qc_df$mode = factor(qc_df$mode, levels = c("wcharlist_sg", "wcharlist_f_sg", "wcharlist_lc_sg", "wcharlist_lg", "wcharlist_f_lg", "wcharlist_lc_lg"))

# ===== Generate QC plots =====

# Labeller for the run mode
mode_lab <- c("Shorter trait list, without follow-up", "Shorter trait list, with follow-up", "Shorter trait list, without follow-up,\nwith LangChain",
              "Longer trait list, without follow-up", "Longer trait list, with follow-up", "Longer trait list, without follow-up,\nwith LangChain")
names(mode_lab) <- c("wcharlist_sg", "wcharlist_f_sg", "wcharlist_lc_sg", "wcharlist_lg", "wcharlist_f_lg", "wcharlist_lc_lg")
...
qc_hist <- ggplot(qc_df, aes(x = prop_recovered)) +
  ...
  facet_wrap(~mode, ncol = 2, labeller = labeller(mode = mode_lab), scales = "free_y", dir = "v") +
  ...
  theme_bw()
qc_hist
ggsave(here::here("figures/langchain_test/wcharlist_prop_recovered_gen_subset.png"), qc_hist, width = 6, height = 5)
...
qc_created_hist <- ggplot(qc_df, aes(x = prop_created)) +
  ...
  theme_bw()
qc_created_hist
ggsave(here::here("figures/langchain_test/wcharlist_prop_created_gen_subset.png"), qc_created_hist, width = 6, height = 5)
