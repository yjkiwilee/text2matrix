...
# QC tsv from desc2matrix_langchain_wcharlist.py
wcharlist_lc_qc <- read_tsv(here::here("../outputs/qc_output/wcharlist/wcharlist_langchain_subset_qc.tsv")) %>%
  dplyr::rename(id = 1) %>% # Rename first column as index
  mutate(mode = "wcharlist_lc")

# Merge dataframes
qc_df <- rbind(wcharlist_qc, wcharlist_f_qc, wcharlist_lc_qc) %>%
...
dplyr::count(qc_df, mode)

# ===== Generate QC plots =====

# Labeller for the run mode
mode_lab <- c("Without follow-up question", "With follow-up question", "Without follow-up question, LangChain")
names(mode_lab) <- c("wcharlist", "wcharlist_f", "wcharlist_lc")
...
qc_hist <- ggplot(qc_df, aes(x = prop_recovered)) +
  ...
  theme_bw()
qc_hist
ggsave(here::here("figures/langchain_test/wcharlist_prop_recovered_subset.png"), qc_hist, width = 5, height = 4)
...
qc_created_hist <- ggplot(qc_df, aes(x = prop_created)) +
  ...
  theme_bw()
qc_created_hist
ggsave(here::here("figures/langchain_test/wcharlist_prop_created_subset.png"), qc_created_hist, width = 5, height = 4)
