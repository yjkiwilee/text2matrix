...
wcharlist_lc_sg_dat <- read_json(here::here("../outputs/wcharlist_output/wcharlist_langchain_sgenlist_subset.json"))
...
wcharlist_lc_lg_dat <- read_json(here::here("../outputs/wcharlist_output/wcharlist_langchain_lgenlist_subset.json"))

# Vector of the method names
method_names <- c("wcharlist_sg", "wcharlist_f_sg", "wcharlist_lc_sg", "wcharlist_lg", "wcharlist_f_lg", "wcharlist_lc_lg")
method_labels <- c("Shorter trait list, without follow-up",
                   "Shorter trait list, with follow-up",
                   "Shorter trait list, without follow-up,\nwith LangChain",
                   "Longer trait list, without follow-up",
                   "Longer trait list, with follow-up",
                   "Longer trait list, without follow-up,\nwith LangChain")
...
charlists_given <- list(
  wcharlist_sg = wcharlist_sg_dat$charlist,
  wcharlist_f_sg = wcharlist_f_sg_dat$charlist,
  wcharlist_lc_sg = wcharlist_lc_sg_dat$metadata$charlist,
  wcharlist_lg = wcharlist_lg_dat$charlist,
  wcharlist_f_lg = wcharlist_f_lg_dat$charlist,
  wcharlist_lc_lg = wcharlist_lc_lg_dat$metadata$charlist
)
...
charlists_actual <- list(
  wcharlist_sg = lapply(wcharlist_sg_dat$data, data2charlist),
  wcharlist_f_sg = lapply(wcharlist_f_sg_dat$data, data2charlist),
  wcharlist_lc_sg = lapply(wcharlist_lc_sg_dat$data, data2charlist),
  wcharlist_lg = lapply(wcharlist_lg_dat$data, data2charlist),
  wcharlist_f_lg = lapply(wcharlist_f_lg_dat$data, data2charlist),
  wcharlist_lc_lg = lapply(wcharlist_lc_lg_dat$data, data2charlist)
)
...
prop_given_histplots <- ggplot(compare_merged_df, aes(x = prop_given_in_actual)) +
  ...
  geom_vline(data = ddply(compare_merged_df, "method", summarize, med_prop = median(prop_given_in_actual, na.rm = TRUE)),
             aes(xintercept = med_prop), linetype = "dashed", color = "red") +
  facet_wrap(~ method, ncol = 2, labeller = labeller(method = method_lab), scales = "free_y", dir = "v") +
...
ggsave(here::here("figures/langchain_test/wcharlist_chars_recovered_gen_subset.png"), prop_given_histplots, width = 6, height = 5)
...
prop_actual_histplots <- ggplot(compare_merged_df, aes(x = prop_actual_in_given)) +
  ...
  labs(
    ...
    y = "Count",
    title = "Proportion of extracted characteristics\nin the original list using traits\ngenerated by accumulation"
  ) +
  theme_bw()
prop_actual_histplots
ggsave(here::here("figures/langchain_test/wcharlist_chars_actual_gen_subset.png"), prop_actual_histplots, width = 6, height = 5)
