...
# Data from desc2matrix_langchain_accum.py
subset_lc_dat <- read_json(here::here("../outputs/accum_output/accum_langchain_subset.json"))
...
accum_dats <- list(
  ...
  subset_lc = subset_lc_dat
)

method_names <- c("subset", "subset_tab", "subset_f", "subset_lc")
method_labels <- c(
...
accum_plt
ggsave(here::here("figures/langchain_test/subset_langchain_accum.png"), accum_plt, width = 6, height = 5.5)
