...
Your goal is to transcribe the given botanical descriptions of plant characteristics into a valid JSON output.
...
# Generate a structured 'table' of JSON from the given species descriptions
def get_table(sys_prompt, prompt, spids, descs, client, model = 'desc2matrix', silent = False):
...
def main(sys_prompt, prompt):
    ...
    parser.add_argument('--numpredict', required = False, type = int, default = 4096, help = 'Maximum number of tokens the model can generate')
    ...
    # Variable to store extracted table
    chars_tab = {}

    get_table(sys_prompt, prompt, descdf['coreid'].tolist(), descs, client, silent = args.silent == True)

    # Store generated outputs
    outdict['data'] = chars_tab
